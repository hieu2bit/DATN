  ...
  const isVariantsListValid = () => {
    ...
    const handleImageUpload = async (colorIndex, file) => {
      ...
      try {
        ...
        // Hiển thị ảnh tạm thời trước khi upload (ảnh theo nhóm màu)
      ...
      variantsList.forEach((variantData) => {
        variantData.variants.forEach((variant) => {
          const newVariant = {
            ...
            photo: variantData.photo  || null, // Hình ảnh (nếu có, nếu không để null)
                    ...
                    <thead>
                      <tr className="bg-gray-100">
                        {["STT", "Sản phẩm", "Giá nhập", "Giá bán", "Số lượng", "Xóa"].map((header) => (
                          <th key={header} className="py-3 px-4 text-center text-gray-500 text-sm font-semibold tracking-wider">
                            {header}
                          </th>
                        ))}
                      </tr>
                      ...
                      {variantData?.variants?.length > 0 ? (
                        <>
                          {variantData.variants.map((variant, index) => (
                            <tr key={`${variant.color}-${variant.size}-${index}`} className="border-b hover:bg-gray-50 text-black text-xs transition-colors">
                          ...
                          {/* Phần upload ảnh theo nhóm màu, đặt dưới bảng */}
                          <tr>
                            <td colSpan="6">
                              <div className="mt-4 bg-blue-50 p-4 rounded-lg border-2 border-dashed border-blue-300 relative">
                                <label className="cursor-pointer">
                                  <input
                                    type="file"
                                    accept="image/*"
                                    onChange={(e) => handleImageUpload(idx, e.target.files[0])} // Chỉ truyền idx (colorIndex)
                                    className="hidden"
                                  />
                                  {variantData.previewImage || variantData.photo ? (
                                    <img
                                      src={variantData.previewImage || variantData.photo}
                                      alt="Ảnh nhóm màu"
                                    ...
                                    <div className="flex flex-col items-center justify-center h-32">
                                      ...
                                      <p className="text-gray-500">
                                        Thả hình ảnh vào đây hoặc <span className="text-blue-500">chọn ảnh</span>
                                      </p>
